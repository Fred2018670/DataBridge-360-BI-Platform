...
(Tracks: Future dates, negative quantities,                     (DimCustomer, DimProduct,
invalid emails, bad phone lengths)                              DimDate ──► FactSales)
│
▼
[Power BI Star Schema Views]


1. **Landing Layer:** Raw data is ingested exactly as-is into `Landing_` tables with loose constraints to ensure zero ingestion failures.
2. **Staging & Validation Layer:** Specialized T-SQL stored procedures execute data cleansing algorithms (text trimming, capitalization, phone/email string validation) and evaluate advanced business logic rules.
3. **Audit & Rejection Layer:** Records violating data integrity rules are safely routed to a central `DataQualityIssues` table with logged violation signatures, ensuring the pipeline never crashes.
4. **Reporting Warehouse:** Validated records are mapped to surrogate keys and processed into a centralized star schema optimized for fast analytical aggregations.
...
* **`01_ddl_schema_setup.sql`:** Provisions the database layout, data types, and primary/foreign key relationships across the landing, staging, and dimensional structures (`DimCustomer`, `DimProduct`, `DimDate`, `FactSales`).
* **`02_etl_stored_procedures.sql`:** Houses the transactional logic engines (`usp_Load_Customers`, `usp_Load_Products`, `usp_Load_Sales`) and the master coordinator wrapper (`usp_Refresh_DataWarehouse`).
* **`03_reporting_views.sql`:** Decouples the presentation layer from the physical database layout, providing high-performance, denormalized structures natively tailored for Power BI DirectQuery/Import processing.
* **`04_mock_data_and_validation.sql`:** Automated testing script that populates the complete 10-year calendar matrix, introduces corrupted sample data payloads, drives the execution pipeline, and prints analytical system health tallies.
...
| **Sales** | `Raw_Quantity` | Blocks negative quantities (`Quantity <= 0`). | Trap structural error, drop order record. |
...
To transition the dashboard from basic column implicit sums to a high-performance, explicit calculation layer, the following specialized **DAX measures** were engineered:
